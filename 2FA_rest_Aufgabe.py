...
def generate_qr_page(text):
    ...
    # Create a simple HTML page to display the QR code and verification form
    html = f"""
    <!doctype html>
    <html lang="en">
      <head>
      ...
      <body>
        ...
        <form action="/verify_totp" method="POST">
          <input type="hidden" name="secret" value="{my_secret}">
...
def verify_totp_page():
    """Handle the TOTP verification from the form."""
    secret = request.form.get('secret')
    current_totp = request.form.get('current_totp')
    
    if not secret or not current_totp:
        return "Secret and current_totp are required", 400
